			...
			if(pszText != NULL)
			{
				if(GetWindowText(hEdit, pszText, dwBufferSize))
				{
					...
					if(WriteFile(hFile, pszText, dwTextLength, &dwWritten, NULL))
						...
						MessageBox(NULL, "Failed1!", "FF", MB_OK);
				}
				else
					MessageBox(NULL, "Failed2!", "FF", MB_OK);
				GlobalFree(pszText);
			}
			else
				MessageBox(NULL, "Failed3!", "FF", MB_OK);
		}
		else
			MessageBox(NULL, "Failed4!", "FF", MB_OK);
	}
	else
		MessageBox(NULL, "Failed5!", "FF", MB_OK);
